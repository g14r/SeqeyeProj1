function ANA=se1_subj(subjname,fig,block,trial)



% ------------------------- Directories -----------------------------------
baseDir         ='/Volumes/MotorControl/data/SeqEye1/SEp/data';
% -------------------------------------------------------------------------
cd (baseDir)
mkdir('analyze');
...
for b  = 1: length(block)
    ...
    for i=1:length(trial) % loop over all trials
        [C]=se1_trial(MOV{D.TN(trial(i))},getrow(D,(D.BN == block(b) & D.TN == trial(i))),fig);
