function CMB = se1_getChunks(MaxChnkLeng,NumDuo,NumTrio,NumQuadro, GenGroupCLADist)
% MaxChnkLeng = 4;
% NumDuo      = 2;
% NumTrio     = 2;
% NumQuadro   = 2;
...
m = randi([1,length(CMB.comb2)] , NumDuo, 1);
% dont allow equal chunks, mirror chunks, or repetition of the same finger chunks
while (CMB.comb2(m(1) , 1) == CMB.comb2(m(1) , 2) | CMB.comb2(m(2) , 1) == CMB.comb2(m(2) , 2) | isequal(CMB.comb2(m(1) , :) , CMB.comb2(m(2) , :)) | CMB.comb2(m(1) , :) == fliplr(CMB.comb2(m(2) , :)))
    m = randi([1,length(CMB.comb2)],NumDuo, 1);
end
CMB.Chunks{2,1}(: , 1:2) = CMB.comb2(m , :);

cmb2 = find(ismember(CMB.comb2 , CMB.Chunks{2,1}(:,1:2) , 'rows')); % indices for the duos of interest in CMB.comb2
...
temp3 = {};
for j  = 1:NumDuo
    temp3{j} = CMB.comb2NotIn3{cmb2(j)};
end
ind = ismember(temp3{1} , temp3{2} , 'rows');
temp3 = temp3{1}(ind , :); % all the trios that dont contain BOTH duos
% for j = 1:length(ind)
%     indId(j) = length(find(temp3(:,1) == ind(j)));
% end
% indId = ind(indId == NumDuo);
% [a , b] = ismember(indId , temp3(:,1));
% temp3 = temp3(b , :);
clear m
